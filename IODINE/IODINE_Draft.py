...
def my_exponential_linear_unit(input):
    ...
    # return (tfk.backend.elu(input) + 1.0 + tfk.backend.epsilon())
...
@tf.function
def mixture_components_generator(logits, components):
    tetramix = tfd.Mixture(
        cat=tfd.Categorical(logits=logits),
        components=[tfd.Normal(loc=components[0,:,:,:], scale=0.1),
                    tfd.Normal(loc=components[1,:,:,:], scale=0.1),
                    tfd.Normal(loc=components[2,:,:,:], scale=0.1),
                    tfd.Normal(loc=components[3,:,:,:], scale=0.1)]    
        )
    return tetramix.sample()

class MixtureDensity(tfkl.Layer):

    # DIFNE LIKELIHOOD BECUASE OF TF.FUNCTION-DECORATOR 
    # YOU CANNOT DEFINE A VARIABLE MORE THAN ONCE IN GRAPH MODE
    ...
    @tf.function
    def call(self, inputs):
        mus, masks_logits = tf.split(inputs, num_or_size_splits=[3,1], axis=-1)
        if self.likelihood is None:
            self.likelihood = tf.Variable(tf.zeros(shape=(mus.shape[0],mus.shape[2],mus.shape[3],mus.shape[4]),
                                              dtype=np.float32))
        masks = tf.nn.softmax(masks_logits, axis=1)
        masks_tiled_transposed = tf.transpose(tfk.backend.tile(masks, (1,1,1,1,3)), [0,2,3,4,1])
        for i in tf.range(tf.shape(mus)[0]):
            self.likelihood[i,:,:,:].assign(mixture_components_generator(logits=masks_tiled_transposed[i,:,:,:,:],
                                                                            components=mus[i,:,:,:,:]))
        return masks_logits, masks, mus, self.likelihood
...
masks_logits, masks, mus, likelihood = MixtureDensity(name='Mixture_Density')(rgb_mask)
...
class VAE(tfk.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            log_true_conditional_density_distribution_loss = log_normal_pdf(posterior, 0.0, 0.0)
            ...
            loss = (1.0/ tf.cast(batch_size, tf.float32)) * (1.0 / tf.cast(self.iterations, dtype=tf.float32)) *  \
                   -tf.reduce_mean(log_likelihood_loss + \
                                   log_true_conditional_density_distribution_loss + \
        ...
        self.optimizer.apply_gradients(zip(generator_grads_clipped, self.generator.trainable_variables))
        ...
        # posterior_gradient_clipped = tf.clip_by_norm(posterior_gradient , 5.0) 
        ...
        for i in tf.range(2, self.iterations + 1, 1):
            ...
            self.optimizer.apply_gradients(zip(refinement_grads_clipped, self.refinement.trainable_variables))
            
            likelihood = self.layer_normalization_likelihood(likelihood)
            posterior_gradient = tape.gradient(loss, posterior)
            posterior_gradient_clipped_layer_normalized = self.layer_normalization_posterior_gradient(posterior_gradient)
            # posterior_gradient_clipped = tf.clip_by_norm(posterior_gradient , 10.0)
            # posterior_gradient_clipped_layer_normalized = self.layer_normalization_posterior_gradient(posterior_gradient_clipped)
...
#     initial_learning_rate, decay_steps=100, decay_rate=.9, staircase=True
#     )
...
#     return tf.reduce_sum(tf.math.square(y_true - y_pred), axis=[1,2,3])

vae = VAE(generator, refinement, image_size=IMAGE_SIZE, latent_dim=latent_dim, iterations=T, num_slots=K)
vae.compile(optimizer=tfk.optimizers.Adam(learning_rate=0.0003),
...
# validation_dataset = dataset.shuffle(2000).batch(batch_size)

for data in batched_dataset.take(1):
    print(data.keys())

# %%
batched_dataset_take = batched_dataset.take(10) 
# batched_valid_take = validation_dataset.take(2)
vae.fit(batched_dataset_take,
